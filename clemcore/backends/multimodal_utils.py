...
Each family of models has a different set of image processing methods, so different inference methods are required
Yet with some tweaks, certain inference methods for different models can be merged
For example - Inference on InternVL2, InternVL2.5, NVLM can be done via ##### INTERNVL2 TYPE MODELS ##### methods

General Structure
Two main methods are required
1) generate_model_prompt_text which should return a string of input prompt (after applying chat template and other such processing steps)
2) generate_model_response which also return a string generated by the model.
These two methods along with keyword args for processor/model should be set in model_registry under model_config key
"""
...
def split_model(model_name):
    ...
    Args:
        model_name (str): The name of the model to be split.
                          Expected values include 'InternVL2', 'InternVL2_5' models
